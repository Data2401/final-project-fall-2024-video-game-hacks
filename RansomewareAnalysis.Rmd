...
```{r plot-wider, fig.width=12, fig.height=13, echo=FALSE} 
most_affected <- RA %>% group_by(sector) %>% summarise(n=n()) %>% arrange(desc(n), sector)

ggplot(most_affected, aes(y=fct_rev(sector), x = n)) + geom_col() + scale_x_continuous() + labs(y=NULL, x = "Number of Attacks")
```
