...
#view(most_affected)
ggplot(most_affected, aes(y=fct_rev(sector), x = n)) + geom_col() + scale_x_continuous() + theme(panel.background = element_rect(color = "white", fill = "gray")) + labs(y=NULL, x = "Number of Attacks") + labs(title = "Top 5 Industries Affected By Ransome Attacks", subtitle = "Ransome Attacks From 2013-2023")
```
...
##Q2
```{r}
view(RA_Attacker_Map) #List of Ransome Countries (WIP)
```
...
```{r}
RA_Victums <- RA_V.2 %>% group_by(location, region) %>% summarise()
view(RA_Victums)
RA_Victums <- 


RA_Victums_Map <- RA_V.2 %>% group_by(location) %>% summarise()
view(RA_Victum_Map)

mapdata. <- map_data("world")


Xmapdata <- left_join(mapdata., RA_Hackers, by="region")
mapdata1 <- mapdata %>% filter(!is.na(mapdata$Ransomware))
map1 <- ggplot(mapdata1, aes(x= long, y = lat, group=group)) +
  geom_polygon(aes(fill = region),fill = "firebrick4", color = "black") + 
  borders(alpha = 0.1, fill = "dodgerblue") +
